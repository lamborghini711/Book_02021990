...
import AddChapter from './../edit-book-select/add-chapter';
...
class BookInfoFooter extends Component {
  ...
  if (this.props.data) {
    ...
    if (this.props.response) {
      item = this.props.response.data_SV1;
    }
      ...
      if(_.get(user,'is_admin', '')){
        add.push(
          <AddChapter key = {1} id={this.props.data.book_id}/>
        )
      }
    }
    ...
    return (
      <div className="row pd-top-10">
        <h3 className="text-600 pd-bottom-10 inline">
...
export default connect(mapStateToProps, null)(BookInfoFooter);